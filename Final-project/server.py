...
def read_html_file(filename):
    contents = Path("html/" + filename).read_text(encoding="utf-8")  # añade esto
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            if data is None:
                self._send_error(want_json, "Could not reach Enseml API. ", 503)
                return
            ...
            if limit_param:
                try:
                    ...
                    self._send_error(want_json, "Could not reach Enseml API. ", 400)
                    return
            ...
            if want_json:
                ...
                contents = read_html_file("karyotype.html").render(
                    context={"species": species, "karyotype": data.get("karyotype", [])})
                self._send_html(contents)
